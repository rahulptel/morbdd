...
struct BDD
{
	...
	// Remove node (not from layer)
	void remove_node_ref_prev(Node *node);
	...
	// Get number of arcs in the BDD
	size_t get_arcs_count();

	// Get number of nodes per layer
	vector<size_t> get_num_nodes_per_layer();

	vector<size_t> get_in_degree();

	vector<size_t> get_max_in_degree_per_layer();
...
// Remove node (not from layer)
//
inline void BDD::remove_node_ref_prev(Node *node)
{
	for (int arc_type = 0; arc_type < 2; ++arc_type)
	{
		// Remove node from incoming arc references
...
inline size_t BDD::get_arcs_count()
{
	size_t num_arcs = 0;
	for (int l = 0; l < num_layers; ++l)
	{
...
inline vector<size_t> BDD::get_in_degree()
{
...
inline vector<size_t> BDD::get_max_in_degree_per_layer()
{
	vector<size_t> max_in_degree_per_layer;
	unsigned int max_in_degree, ind;
	for (int l = 0; l < num_layers; ++l)
	{
		max_in_degree = 0;
		for (int m = 0; m < layers[l].size(); ++m)
		{
			ind = layers[l][m]->prev[0].size() + layers[l][m]->prev[1].size();
			max_in_degree = std::max(max_in_degree, ind);
		}

		max_in_degree_per_layer.push_back((size_t)max_in_degree);
	}

	return max_in_degree_per_layer;
}

inline vector<size_t> BDD::get_num_nodes_per_layer()
{
	vector<size_t> nodes_per_layer;
	for (int l = 0; l < num_layers; ++l)
	{
		nodes_per_layer.push_back(layers[l].size());
	}
