...
struct SetCoveringInstance
{
	...
	SetCoveringInstance(int n_vars, int n_cons, int n_objs,
						vector<vector<int>> objs,
						vector<vector<int>> vars_cons);
...
inline SetCoveringInstance::SetCoveringInstance(int _n_vars,
												int _n_cons,
												int _n_objs,
												vector<vector<int>> _objs,
												vector<vector<int>> _vars_cons)
{
	n_vars = _n_vars;
	n_cons = _n_cons;
	n_objs = _n_objs;
	...
	for (int c = 0; c < vars_cons.size(); ++c)
	{
