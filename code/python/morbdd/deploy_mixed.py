...
def save_bdd_data(cfg, pids, bdd_data, mdl_hex):
    out_path = resource_path / f"predictions/{cfg.deploy.mdl}/{cfg.prob.name}/mixed/{cfg.prob.size}/{cfg.deploy.split}/{mdl_hex}"
    if cfg.deploy.stitching_heuristic == "min_resistance":
...
def worker(rank, cfg, mdl_hex):
    ...
    for pid in range(cfg.deploy.from_pid + rank, cfg.deploy.to_pid, cfg.deploy.num_processes):
        ...
        # Load BDD
        bdd_path = resource_path / (
            f"predictions/{cfg.deploy.mdl}/{cfg.prob.name}/mixed/{cfg.prob.size}/{cfg.deploy.split}/"
