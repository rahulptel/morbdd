...
def _get_pareto_state_scores(x):
    x = np.array(x)
    x = x[:, 1:]
...
def _tag_dd_nodes(pid, dd, pareto_state_scores, save_all_neg=False):
...
def run_pipeline(
    ...
    if true_pf is not None:
        res = metric_calculator.compute_cardinality(true_pf["z"], result.pred_pf)
        result.cardinality_raw = res["cardinality_raw"]
        result.cardinality = res["cardinality"]
        result.precision = res["precision"]

    rest_dd = env.get_dd()
    pareto_state_scores = _get_pareto_state_scores(true_pf["x"])
    result = _tag_dd_nodes(0, rest_dd, pareto_state_scores, save_all_neg=False)
    total, one = 0, 0
    for r in result:
        total += len(r)
        one += np.sum(r)
    print("Precision: ", one / total)
    return result
...
def main(cfg):
    ...
    for pid in range(cfg.from_pid, cfg.to_pid):
        ...
        exact_dd_dict = []
        for layer in exact_dd:
            _exact_dd_dict = []
            for nid, n in enumerate(layer):
                _exact_dd_dict.append({"nid": nid, "n": n})
            exact_dd_dict.append(_exact_dd_dict)
        # pareto_state_scores = _get_pareto_state_scores(cfg, None, true_pf["x"])
        # labels = _tag_dd_nodes(pid, exact_dd, pareto_state_scores, save_all_neg=False)
