...
class GATEncoderLayer(nn.Module):
    def __init__(self, d_emb, n_heads=8, dropout=0.0):
        super(GATEncoderLayer, self).__init__()
        # GATv2Conv allows edge attributes
        self.gat_conv = GATv2Conv(
            d_emb, d_emb // n_heads, heads=n_heads, dropout=dropout, edge_dim=d_emb
        )
...
class GATEncoder(nn.Module):
    def __init__(self, d_emb=32, n_layers=2, n_heads=8, dropout=0.0):
        super(GATEncoder, self).__init__()
        self.encoder_layers = nn.ModuleList(
            [
                GATEncoderLayer(d_emb=d_emb, n_heads=n_heads, dropout=dropout)
...
class ParetoNodePredictor(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.token_encoder = TokenEmbedGraph(d_emb=self.cfg.d_emb)
        self.graph_encoder = None
        if self.cfg.type == "gtf":
            self.graph_encoder = GTEncoder(
                d_emb=self.cfg.d_emb,
                n_layers=self.cfg.n_layers,
                n_heads=self.cfg.n_heads,
                bias_mha=self.cfg.bias_mha,
                dropout_attn=self.cfg.dropout_attn,
                dropout_proj=self.cfg.dropout_proj,
                bias_mlp=self.cfg.bias_mlp,
                dropout_mlp=self.cfg.dropout_mlp,
                h2i_ratio=self.cfg.h2i_ratio,
            )
        # elif self.cfg.type == "gat":
        ...
        self.visit_encoder = nn.Embedding(self.NODE_VISIT_TYPES, self.cfg.d_emb)
        self.node_visit_encoder1 = nn.Sequential(
            nn.Linear(self.cfg.d_emb, self.cfg.h2i_ratio * self.cfg.d_emb),
            nn.ReLU(),
        )
        self.node_visit_encoder2 = nn.Sequential(
            nn.Linear(self.cfg.h2i_ratio * self.cfg.d_emb, self.cfg.d_emb),
            nn.ReLU(),
        )
        self.layer_encoder = nn.Sequential(
            nn.Linear(self.N_LAYER_INDEX, self.cfg.d_emb),
            nn.ReLU(),
        )
        self.pareto_predictor = nn.Sequential(
            nn.Linear(self.cfg.d_emb, self.cfg.h2i_ratio * self.cfg.d_emb),
            nn.ReLU(),
            nn.Linear(self.cfg.h2i_ratio * self.cfg.d_emb, self.N_CLASSES),
        )

    def forward(self, n, e, l, s):
        # Transform input n and e features
        # n: B x n_objs x n_vars x 2 -> B x n_vars x d_emb
        # e: B x n_objs x n_vars x n_vars x 1 -> B x n_vars x n_vars x d_emb
...
def print_eval_result(
    split, ep, max_epochs, global_step, max_steps, result, verbose=True
):
    print(
...
def adjust_learning_rate(step, optimizer, scheduler, val_metric, max_lr, warmup_steps):
    """Linearly increase learning rate and then decrease the learning rate using a scheduler."""
...
def training_loop(
    ...
    if cfg.decay == "Cosine":
        scheduler = CosineAnnealingLR(
            optimizer, max_steps - warmup_steps, cfg.max_lr / 10
        )
        ...
        scheduler = None
    print(
        "Training epochs: {}, max steps: {}, warm-up steps: {}/{:.2f}".format(
            cfg.epochs, max_steps, warmup_steps, warmup_steps / max_steps
        )
    )
    ...
    batch_load_time, epoch_time = 0, 0
    epoch_count = 0
    ...
    for ep in range(cfg.epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            adjust_learning_rate(
                global_step, optimizer, scheduler, val_metric, cfg.max_lr, warmup_steps
            )
            ...
            if global_step % cfg.eval_every == 0:
                ...
                if is_better(best_metric, val_result[metric_type], metric_type):
                    ...
                    save_path = open(str(exp_path / f"best_result.pkl"), "wb")
                    save_result(save_path, ep, global_step, train_result, val_result)

                print(
                    "\t{}Best epoch:step={}:{}, Best {}: {}".format(
                        "* " if best_step == global_step else "",
                        best_epoch,
                        best_step,
                        metric_type,
                        best_metric,
            ...
            if global_step == max_steps:
                return batch_load_time / global_step, epoch_time / epoch_count

        epoch_time += time.time() - start
        epoch_count += 1

        print("Batch time: {:6.f}".format(batch_load_time / global_step))
        print("Epoch time: {:6.f}".format(epoch_time / epoch_count))
...
def main(cfg):
    ...
    batch_time, epoch_time = training_loop(
        cfg, model, opt, loss_fn, train_dataloader, val_dataloader, metric_type="f1"
    )
    print("Finished training...")
    print("Batch time: {:6.f}".format(batch_time))
    print("Epoch time: {:6.f}".format(epoch_time))
