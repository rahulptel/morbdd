...
def generate_instance(rng, n_vars, n_objs, max_obj=1000):
    ...
    for _ in range(n_objs):
        ...
        for i in range(n_vars):
            lb = max(1, data['weight'][i] - (max_obj / 10))
            ub = data['weight'][i] + (max_obj / 10)
...
@hydra.main(version_base='1.2', config_path='./configs', config_name='generate_instance.yaml')
def main(cfg: DictConfig):
    ...
    for s in cfg.size:
        ...
        for id in range(cfg.n_train):
            write_to_file(
                resource_path / 'instances' / f'{cfg.name}/{n_objs}_{n_vars}/train/kp_{cfg.seed}_{n_objs}_{n_vars}_{id}.dat',
        ...
        for id in range(start, end):
            write_to_file(
                resource_path / 'instances' / f'{cfg.name}/{n_objs}_{n_vars}/val/kp_{cfg.seed}_{n_objs}_{n_vars}_{id}.dat',
                generate_instance(rng, n_vars, n_objs, max_obj=cfg.max_obj))
        ...
        for id in range(start, end):
            write_to_file(
                resource_path / 'instances' / f'{cfg.name}/{n_objs}_{n_vars}/test/kp_{cfg.seed}_{n_objs}_{n_vars}_{id}.dat',
                generate_instance(rng, n_vars, n_objs, max_obj=cfg.max_obj))
