...
def get_xgb_mixed_model_name(max_depth=None,
                             eta=None,
                             ...
                             device=None):
    def get_mixed_model_name_knapsack():
        name = ""
    ...
    if prob_name == "knapsack":
        return get_mixed_model_name_knapsack()
    else:
        raise ValueError("Invalid problem!")


class Iterator(xgb.DataIter):
    def __init__(self, problem, dataset_type, split, neg_pos_ratio, min_samples, sampling_type, weights_type,
                 labels_type,
                 names):
        ...
        self.zf_sampling_type = zipfile.ZipFile(
            resource_path / f"xgb_data/{self.problem}/{self.dataset_type}/{self.split}/{sampling_type}.zip")
        self.zf_labels_type = zipfile.ZipFile(
            resource_path / f"xgb_data/{self.problem}/{self.dataset_type}/{self.split}/labels/{labels_type}.zip")
...
def get_iterator(cfg, sampling_type, weights_type, label_type, split):
    ...
    dataset_type = "mixed"
    zf_path = zipfile.Path(resource_path / f"xgb_data/knapsack/{dataset_type}/{split}/{sampling_type}.zip")
...
def main(cfg):
    ...
    # Get model name
    mdl_path = resource_path / f"pretrained/xgb/{cfg.prob.name}/mixed"
    mdl_path.mkdir(parents=True, exist_ok=True)
    mdl_name = get_xgb_mixed_model_name(max_depth=cfg.max_depth,
                                        eta=cfg.eta,
                                        min_child_weight=cfg.min_child_weight,
                                        subsample=cfg.subsample,
                                        colsample_bytree=cfg.colsample_bytree,
